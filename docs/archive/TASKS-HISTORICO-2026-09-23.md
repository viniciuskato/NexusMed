> **Arquivo morto (desde 2026-09-23).** Este é o `TASKS.md` como estava
> até 2026-09-23, preservado sem cortes (só as linhas 05 e 06 de 23/09
> foram atualizadas para "mesclado"). A fila atual, só com o que está
> aberto, está em [`docs/operacao/TASKS.md`](../operacao/TASKS.md). Links
> relativos daqui foram escritos para `docs/operacao/`.
...
> [`SESSION_PROTOCOL.md`](../operacao/SESSION_PROTOCOL.md).
...
| TASK-2026-09-23-05 — Padrão de conteúdos autocontido, para entregar a uma IA | P1 | **concluído e mesclado** — PR #66 (`83aa21f`) | Nenhuma | Nenhum no app — só documentação | `PADRAO-NEXUSMED-CONTEUDOS.md` reescrito em duas partes: Parte 1 para quem escreve (pessoa ou IA sem contexto — níveis da árvore, profundidade, citações, formato `.md`, limites do leitor, checklist) e Parte 2 para quem opera (importar, posicionar, revisar/atestar, publicar, vincular questões). Alinhado ao ciclo de estudo: não preencher "Tipo do nó"/"Estude antes"/"Veja também"; tags como palavras-chave. Exemplo do bloco de formato rodado pelo importador real (2 seções, Pontos-Chave, Pérola, Alerta, tags, 2 referências, nenhum campo faltando). Regra de manutenção em AGENTS.md, risco 19 |
