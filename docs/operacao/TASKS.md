...
| TASK-2026-09-22-02 — Taxonomia hierárquica de materiais, piloto antibióticos/β-lactâmicos | P1 | **Fases 1 e 1.5 implementadas e validadas localmente; aguardando revisão independente** — branch `work/antibioticos-taxonomia-fase1`, sem push/PR | Aprovação da Fase 1.5 antes de iniciar Admin (Fase 2) | Nenhum: Supabase remoto, `main` e produção não foram tocados | Revisar as duas migrations, as RPCs e os 51 testes pgTAP de `material_taxonomy.test.sql`; antes de qualquer publicação, inventariar `material_links` no remoto por leitura — a migration 1.5 **falha de propósito** se algum par de materiais tiver ligação nos dois sentidos, herança da constraint antiga de `material_dependencies`. Abrir PR e aplicar as duas migrations no remoto antes do merge, conforme RUNBOOK. Depois, autorizar separadamente a Fase 2 (Admin) |
| TASK-2026-09-22-03 — Fase 1.5: revisão da fundação da taxonomia antes de produzir conteúdo | P1 | **concluído localmente, não publicado** — mesma branch da 02 | Nenhuma | Nenhum: só código local e Supabase local | Corrigidos, contra o Supabase local: hash de atestação voltou a cobrir só conteúdo; pai/filho amarrados por disciplina (tema livre); teto de 8 níveis e guarda de ciclo nas travessias; exclusão determinística dos dois lados de `Veja também`; um par com uma ligação só; ancestral recusado como `Estude antes`; `nav_short_title`/`taxonomy_kind`; RPC `set_material_position`; exclusão e publicação em lote no Admin com mensagem acionável. Regras em [`standards/taxonomia-materiais.md`](standards/taxonomia-materiais.md). **Aberto**: escopo de questões por nó da árvore e leitura sob demanda (ver seção 7 do standard) — dependem do inventário remoto de `questions.material_id` |
