...
| TASK-2026-09-21-02 — Filtro de status (Todas/Publicadas/Não publicadas) na listagem de Questões Comentadas do Admin | P2 | **implementado, gates locais verdes (typecheck/lint/build) + novo E2E `admin-questoes-filtro-status.spec.ts` passando contra Supabase local, não commitado** | Nenhuma | Nenhum — só frontend (filtra array já carregado), sem migration/RPC nova | Commitar junto com o restante desta branch e abrir PR; sem pendência de infraestrutura |
