...
| TASK-2026-09-22-02 — Taxonomia hierárquica de materiais, piloto antibióticos/β-lactâmicos | P1 | **Fase 1 implementada e validada localmente; aguardando revisão independente** — branch `work/antibioticos-taxonomia-fase1`, ainda sem commit/PR | Aprovação da Fase 1 antes de iniciar Admin (Fase 2) | Nenhum: Supabase remoto, `main` e produção não foram tocados | Revisar migration/RPC/pgTAP; antes de qualquer publicação, inventariar `material_dependencies` no remoto por leitura, abrir PR e aplicar a migration remota antes do merge conforme RUNBOOK. Depois, autorizar separadamente a Fase 2 (Admin) |
