...
| Missão AS1-B3.2 — Correção da sobreposição visual do importador e retificação da matriz científica residual | P1 | **executado — aguardando verificação da diretoria** — branch local `work/as1-b3-2-correcao-visual-importador-acidobasico`, worktree `.claude/worktrees/as1-b3-2-correcao-visual`, a partir de `work/as1-b3-1-retificacao-qa-acidobasico`@`e6320f2` | Depende da AS1-B3.1 (executada) | Nenhum — corrigida a matriz científica residual do SODa-BIC (linha da matriz da seção 2 do relatório, sem alteração do YAML); corrigida a descrição do QA anterior (seção 10 do relatório) para registrar a sobreposição encontrada; `ImportMaterialModal.tsx` agora renderiza num overlay `fixed inset-0 z-[60]` (acima do dock, `z-40`), eliminando a sobreposição; teste de regressão E2E adicionado em `tests/e2e/specs/import-material.spec.ts` (2 casos: desktop 1400×1000 e mobile 390×844, prova por bounding box + `elementFromPoint`, com controle negativo confirmado — o teste falha sem a correção); typecheck/lint/testes unitários e de componente/E2E de importação/build/gate sem debug todos verdes; evidências substituídas/adicionadas em `docs/editorial/as1/evidencias/` (desktop e mobile); YAML não tocado; Supabase remoto/produção/main/Vercel não tocados; nada importado/publicado | Diretoria verifica esta execução; decisões de taxonomia ("Nefrologia"/"Distúrbio Acidobásico") e autorização de importação real seguem pendentes, como já registrado na AS1-B3.1 |
