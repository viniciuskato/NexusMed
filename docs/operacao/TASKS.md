...
| Missão 42-A — Entrada assistida de materiais ("Importar material" no CMS) | P1 | **superada pela 42-B** — ver nota abaixo | Nenhuma | Nenhum — `work/42a-import-assistido` não foi enviada ao remoto nem mesclada em `main` | Nenhuma isolada — avaliar junto com 42-B |
| Missão 42-B — Corrigir gravação da importação para atômica (RPC `import_compendium_draft`) | P1 | **concluído tecnicamente, branch candidata só local** | Depende da 42-A (mesma branch) | Nenhum — `work/42a-import-assistido` não foi enviada ao remoto nem mesclada em `main` | Diretoria decide: revisar diff (18 arquivos, 42-A+42-B) e autorizar push da branch candidata / merge em `main` |
