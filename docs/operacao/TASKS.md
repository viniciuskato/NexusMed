...
| TASK-2026-09-21-04 — Estender o filtro de status (TASK-2026-09-21-02) para a listagem de Conteúdos & Mecanismos e para a de Usuários no Admin | P2 | **implementado, gates locais verdes (typecheck/lint) + novo E2E `admin-conteudos-filtro-status.spec.ts` passando contra Supabase local, não commitado** | Nenhuma | Nenhum — só frontend (filtra arrays já carregados), sem migration/RPC nova | Commitar junto com o restante desta branch e abrir PR; sem pendência de infraestrutura |
