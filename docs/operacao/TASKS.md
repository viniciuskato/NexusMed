...
| TASK-2026-09-23-03 — "Salvar" sem perda e importação já posicionada na árvore | P1 | **implementado e validado localmente, aguardando revisão** — branch `work/processo-importacao-arvore`, a partir de `main`@`612b72d` | Nenhuma | **Tem migration** (`20260923120000`): aplicar no remoto ANTES do merge. Medido em produção (só leitura): 0 referências vinculadas, 0 materiais com rótulo curto ou pai — nenhuma aprovação existente muda de hash; 3 revisões aprovadas passam a sobreviver a um "Salvar" sem mudança | Três falhas medidas no Supabase local com o payload exato do formulário: um "Salvar" sem mudar nada mudava o hash atestado, recriava todas as referências com ids novos e **apagava o vínculo com fonte curada**. Além disso o formulário trocava `mode` nulo por `mecanismos` (os 38 materiais de produção têm mode nulo) e apagava `study_lens`. Corrigido no banco (`save_compendium` casa referências por texto e nunca mexe no vínculo) e no cliente (`src/utils/compendiumForm.ts`, ida e volta sem perda, com controle negativo). `nav_short_title` saiu do hash. Processo: `import_compendium_draft` aceita posição e ligações na mesma transação; o modal de importação ganhou o bloco "Posição na árvore" (mesmo componente do formulário), erro com a causa real e "Voltar e corrigir" sem reenviar arquivo, e sucesso com a trilha e os próximos passos; o cartão do Admin mostra "Na árvore" e "Publique antes, nesta ordem"; botão "Publicar Conteúdo" (que só criava rascunho) virou "Salvar rascunho". `publish_material` agora lista a ordem completa de publicação — corrige a Fase 1.5, que apontava só o ancestral mais próximo. Validação: pgTAP 398/398 (23 novos), Vitest 257/257 (26 novos), E2E novo do fluxo completo + import existente 7/7 sem resíduo |
...
| TASK-2026-09-23-01 — Taxonomia, Fase 3: navegação da árvore para o estudante | P1 | **implementado e validado localmente, aguardando revisão** — branch `work/taxonomia-fase3-estudante`, a partir de `main`@`8d1232e` | Nenhuma | Nenhum: sem migration, só frontend contra o schema já em produção | Trilha de navegação clicável no leitor (colapsável no celular, usa `nav_short_title`), cartões `Aprofunde-se` derivados dos filhos, caixas `Estude antes`/`Veja também` visualmente distintas, e terceiro modo de exibição da biblioteca (árvore recolhível) sobre o resultado já filtrado. Tudo em `MaterialNavigation.tsx`, derivando do utilitário puro da Fase 2. Validação: 13 testes de componente novos + E2E novo de 4 casos (árvore → folha → volta pela trilha; as duas caixas; link para material despublicado desaparece; viewport 390px sem rolagem horizontal) contra Supabase local, zero fixture residual. **Dois achados corrigidos no caminho**: `deleteE2EMaterials()` falhava com link `prerequisite` (FK restrict) e deixava resíduo — 60 materiais acumulados; e o `aria-label` do botão de expandir usava o título completo enquanto o item visível usava o rótulo curto. **Achado pré-existente NÃO corrigido** (fora do escopo): em 390px o widget "Plantão de Foco" cobre o primeiro item do menu Recursos do dock inferior |
| TASK-2026-09-22-04 — Inventário remoto somente leitura, pré-requisito da 02 | P1 | **concluído em 2026-09-22** | Nenhuma | Nenhum — só SELECT via `SUPABASE_SERVICE_ROLE_KEY`/PostgREST, nenhuma escrita | Resultado: `material_dependencies` remoto tem 14 linhas, 14 pares distintos, **zero pares nos dois sentidos** — a migration 1.5 não falha por isso, pode aplicar direto. `questions.material_id` preenchido em **9 de 420** questões (2,1%) — confirma que o escopo de questões por nó da árvore (seção 7 do standard) não pode substituir o filtro por tema sem uma vinculação editorial em massa antes; fica registrado como bloqueio explícito, não como código a escrever agora. `material_sections.content` remoto soma ~947 mil caracteres em 836 seções (38 materiais) — ~1 MB no payload de `getCompendiums()` hoje; cresce com a árvore, mantém a leitura sob demanda como item da Fase 3 |
