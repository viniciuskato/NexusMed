...
| TASK-2026-09-21-01 — Importação de questões em lote ("Importar questões", RPC `import_question_draft`) | P1 | **implementado, gates locais verdes (ver PROJECT_STATE.md), PR em preparação** | Nenhuma | Nenhum ainda — branch local `feat/questoes-markdown-previa-busca-overflow`, migration não aplicada no remoto | Abrir PR contra `main`; antes do merge, aplicar `20260921120000_import_question_draft.sql` no Supabase remoto (RUNBOOK seção 3) e rodar `npm run test:e2e`, não executado nesta entrega |
