...
| TASK-2026-09-22-02 — Taxonomia hierárquica de materiais, piloto antibióticos/β-lactâmicos | P1 | **Fases 1 e 1.5 implementadas e validadas; revisão ultra concluída (2 achados corrigidos); inventário remoto concluído sem bloqueio** — branch `work/antibioticos-taxonomia-fase1`@`3d3672c`, publicada em `origin/`, sem PR ainda | Nenhuma restante antes do PR | Nenhum: Supabase remoto, `main` e produção só foram lidos (SELECT via service_role, ver inventário abaixo), nada alterado | Abrir PR, aplicar as duas migrations no remoto (`20260922120000` + `20260922130000`) antes do merge, conforme RUNBOOK, depois autorizar separadamente a Fase 2 (Admin) |
